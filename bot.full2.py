...
# Очередь для хранения сообщений с изображениями (храним file_id и caption)
image_queue = deque()
...
async def send_images(context, count: int) -> None:
    ...
    while image_queue and image_count < count:
        image_data = image_queue.popleft()  # Получаем данные старейшего изображения (file_id и caption)
...
async def handle_new_image(update: Update, context) -> None:
    ...
    if update.channel_post and update.channel_post.chat_id == SOURCE_CHANNEL_ID and update.channel_post.photo:
        # Сохраняем file_id самого большого фото и caption (если есть)
        image_data = {
